    ...
    RefreshTokenInterface
{
    ...
    public function checkUserCredentials($username, $password)
    {
        ...
        $bransArray = $UsersClientController->list_brand_client($_POST["client_id"]);
        if($bransArray !== false){
            $_POST["bransArray"] = $bransArray;
            if ($user = $this->getUser($username,$bransArray)) {
    ...
    //jorge
    public function getUserDetails($username,$bransArray = null)
    {
        if(isset($_POST["bransArray"])){
            return $this->getUser($username,$_POST["bransArray"]);
        }
        return $this->getUser($username,$bransArray);
    }
    ...
    // jorge
    public function getUser($username,$bransArray = null)
    {
        if(isset($bransArray)){
            $stringBrands = implode(",",$bransArray);
            $stmt = $this->db->prepare($sql = sprintf('SELECT id,email,password from %s where email=:username and brand_id in ('.$stringBrands.') and status=1', $this->config['user_table']));
