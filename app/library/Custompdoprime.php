 ...
 */
class Custompdoprime implements
AuthorizationCodeInterface, AccessTokenInterface, ClientCredentialsInterface, UserCredentialsInterface, RefreshTokenInterface {
    ...
    protected function checkPassword($user, $password) {
        ...
        $ctrl = false;
        if($base->validatePassword($password, $user["password"])){
            $plans = $base->SearchPlanActive(array(), $user["user_id"]);
            if ($plans["id_cateogory"] == 0) {
                $data = array(
                    "error" => "invalid_plans",
                    "error_description" => "Does not have an active plan"
                );
            ...
            return $base->registerLastLogin($user); 
            //return true;
        }
