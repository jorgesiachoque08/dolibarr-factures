    ...
    RefreshTokenInterface
{
    ...
    public function getUser($username,$id_organization = null)
    {
        if(isset($id_organization)){
            ...
            $stmt = $this->db->prepare($sql = sprintf('SELECT id,email,password from %s where email=:username and id_organization=:id_organization and is_active=1', $this->config['user_table']));
            $stmt->execute(array('username' => $username,'id_organization' => $id_organization));
        }else{
            $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where email=:username and is_active=1' , $this->config['user_table']));
